...
router.get('/:orderdID',checkAuth ,(req, res, next) => {
    Order.findById(req.params.orderdID)
    .populate('product')
    .exec()
    .then( order => {
        if (!order) {
...
router.delete('/:orderdID',checkAuth ,(req, res, next) => {
    Order.remove({_id: req.params.orderdID})
    .exec()
    .then( result => {
        res.status(200).json({
